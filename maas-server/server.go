...
func main() {
	...
	if err == nil {
		...
		// construct the repositories storing the matomatInstance data
		userRepo, itemRepo, itemStatsRepo, userItemStatsRepo := buildRepos(cfg)
		// construct the authentication handling
		authentication := buildAuth(cfg)
		// construct the user handling
		userManagement := buildUsers(cfg, userRepo)
		// construct the event dispatcher (which allows passing around events generated by the matomat)
		eventDispatcherMqtt := buildEventDispatcher(cfg)

		// build the "actual" matomat, the struct that holds all the matomat business logic
